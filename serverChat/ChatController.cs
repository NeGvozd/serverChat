...
namespace serverChat
{
    public static class ChatController
    {
        ...
        public static void AddMessage(string userName, string msg1, bool isPrivate)
        {
            string msg = msg1;
            string nameof_recipient=null;
            try
            {
                if (isPrivate)
                {
                    MatchCollection _recipient = Regex.Matches(msg, @"((\w*))");
                    nameof_recipient = _recipient[1].Value;
                    msg = msg1.Split(')')[1];
                    msg = "(personally)" + msg;
                }
                ...
                Server.UpdateAllChats(nameof_recipient);
            }
